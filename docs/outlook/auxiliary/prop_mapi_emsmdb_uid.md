...
Utilisez [PROP_ACCT_IS_EXCH](prop_acct_is_exch.md) pour vérifier s'il s'agit d'un compte Exchange. Si c'est le cas, **prop\_MAPI_EMSMDB_UID** est un **ACCT_BIN** qui contient l' **emsmdbUID**, qui est l'ID unique, pour le compte Exchange. Si le compte n'est pas un compte Exchange, cette propriété n'est pas définie.
