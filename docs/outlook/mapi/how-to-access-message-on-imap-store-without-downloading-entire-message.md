...
Cette rubrique présente un exemple de code en C++ qui interroge une banque de messages pour l'interface **[IProxyStoreObject](iproxystoreobject.md)** et utilise le pointeur renvoyé et la fonction **[IProxyStoreObject:: UnwrapNoRef](iproxystoreobject-unwrapnoref.md)** pour obtenir un pointeur vers un objet banque IMAP qui a été dévoilé. L'utilisation de ce magasin non encapsulé autorise l'accès à un message dans son état actuel sans appeler un téléchargement de l'intégralité du message. 
  
Étant donné que **UnwrapNoRef** n'incrémente pas le décompte de référence pour ce nouveau pointeur vers l'objet Store non encapsulé, après avoir correctement appelé **UnwrapNoRef**, vous devez appeler [IUnknown:: AddRef](https://msdn.microsoft.com/library/ms691379%28VS.85%29.aspx) pour maintenir le nombre de références. 
