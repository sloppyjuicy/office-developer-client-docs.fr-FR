...
En plus de ces colonnes requises, les tables de hiérarchie de carnet d’adresses doivent inclure la PR_CONTAINER_FLAGS de carnet **d’adresses.** **PR_CONTAINER_FLAGS** différents attributs d’un conteneur dans la hiérarchie et est utilisé pour distinguer un conteneur d’un autre. 
