...
<span data-ttu-id="c95ae-162">**IStreamDocfile** est un dérivé de l’interface [IStream](http://msdn.microsoft.com/fr-fr/library/aa380034%28VS.85%29.aspx) basé sur un fichier composé OLE 2.0.</span><span class="sxs-lookup"><span data-stu-id="c95ae-162">**IStreamDocfile** is a derivative of the [IStream](http://msdn.microsoft.com/fr-fr/library/aa380034%28VS.85%29.aspx) interface that is based on an OLE 2.0 compound file.</span></span> <span data-ttu-id="c95ae-163">**IStreamDocfile** est la meilleure solution pour l’accès aux pièces jointes OLE 2.0 car elle implique le moins de surcharge.</span><span class="sxs-lookup"><span data-stu-id="c95ae-163">**IStreamDocfile** is the best choice for accessing OLE 2.0 attachments because it involves the least amount of overhead.</span></span> <span data-ttu-id="c95ae-164">Vous pouvez utiliser IID_IStreamDocFile pour ces propriétés qui contiennent des données stockées dans le stockage structuré disponible par le biais de l’interface [IStorage](http://msdn.microsoft.com/fr-fr/library/aa380015%28VS.85%29.aspx) .</span><span class="sxs-lookup"><span data-stu-id="c95ae-164">You can use IID_IStreamDocFile for those properties that contain data stored in structured storage available through the [IStorage](http://msdn.microsoft.com/fr-fr/library/aa380015%28VS.85%29.aspx) interface.</span></span> 
...
<span data-ttu-id="c95ae-166">N’utilisez pas le pointeur **IStream** que vous recevez pour appeler la méthode son [Seek](http://msdn.microsoft.com/fr-fr/library/aa380043%28v=VS.85%29.aspx) ou [SetSize](http://msdn.microsoft.com/fr-fr/library/aa380044%28v=VS.85%29.aspx) sauf si vous utilisez une variable zéro position ou la taille.</span><span class="sxs-lookup"><span data-stu-id="c95ae-166">Do not use the **IStream** pointer that you receive to call either its [Seek](http://msdn.microsoft.com/fr-fr/library/aa380043%28v=VS.85%29.aspx) or [SetSize](http://msdn.microsoft.com/fr-fr/library/aa380044%28v=VS.85%29.aspx) method unless you use a zero position or size variable.</span></span> <span data-ttu-id="c95ae-167">En outre, ne comptez pas sur la valeur du paramètre de sortie _plibNewPosition_ renvoyé à partir de l’appel de la **méthode Seek** .</span><span class="sxs-lookup"><span data-stu-id="c95ae-167">Also, do not rely on the value of the  _plibNewPosition_ output parameter returned from the **Seek** call.</span></span> 
