...
Contient le nom et l'extension du fichier de base d'une pièce jointe, à l'exclusion du chemin.
...
Il est recommandé que les objets Attachment exposent ces propriétés qui se rapportent aux valeurs **ATTACH_BY_VALUE**, **ATTACH_BY_REFERENCE**, **ATTACH_BY_REF_RESOLVE**et **ATTACH_BY_REF_ONLY** de l' **PR_ATTACH_METHOD** ([PidTagAttachMethod](pidtagattachmethod-canonical-property.md)). **PR_ATTACH_FILENAME** et les propriétés associées sont obligatoires lorsqu'une de ces valeurs est utilisée. 
  
Ces propriétés peuvent être utilisées comme nom de fichier suggéré pour enregistrer la pièce jointe et pour fournir l'extension de nom de fichier si la propriété **PR_ATTACH_EXTENSION** ([PidTagAttachExtension](pidtagattachextension-canonical-property.md)) n'est pas fournie. 
...
> Spécifie les propriétés des messages chiffrés et signés S/MIME.
