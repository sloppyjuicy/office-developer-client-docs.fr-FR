...
Pour définir la valeur de cette propriété, les clients Message Extensions MIME (Multipurpose Internet) doivent écrire un champ d’en-tête Accept-Language avec la valeur souhaitée. Clients MIME peuvent écrire un champ d’en-tête X-Accept-Language. Lecteurs MIME doivent copier la valeur d’un champ d’en-tête à la valeur de cette propriété. Si les deux champs d’en-tête sont présentes, lecteurs MIME doivent utiliser le champ d’en-tête Accept-Language.
