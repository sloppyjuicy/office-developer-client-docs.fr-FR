...
Il existe de nombreuses macros dans le fichier d’en-tête Mapidefs.h et fichiers d’en-tête OLE clients et fournisseurs de services dans une langue peuvent utiliser pour leur permettre de leurs définitions d’objets MAPI. Par exemple, il existe une macro pour définir les méthodes de chaque interface MAPI. La macro pour définir les méthodes de l’interface [IUnknown](http://msdn.microsoft.com/en-us/library/ms680509%28v=VS.85%29.aspx) apparaît dans Mapidefs.h comme suit : 
