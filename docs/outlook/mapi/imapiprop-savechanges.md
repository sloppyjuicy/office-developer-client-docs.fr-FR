...
Si une KEEP_OPEN_READWRITE ou KEEP_OPEN_READONLY est définie avec MAPI_DEFERRED_ERRORS, vous pouvez ignorer la demande de report d’erreur. Si MAPI_DEFERRED_ERRORS n’est pas définie dans _ulFlags,_ l’une des erreurs différées précédemment peut être renvoyée pour l’appel **SaveChanges.** 
