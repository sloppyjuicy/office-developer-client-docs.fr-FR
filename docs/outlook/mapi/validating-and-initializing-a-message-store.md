...
Ces propriétés sont des masques de bits qui décrivent les fonctionnalités de la boutique de messages. **PR_VALID_FOLDER_MASK** a un bit pour chaque dossier spécial qui existe dans la magasin de messages et possède un identificateur d’entrée attribué valide. Pour plus d’informations sur l’accès à ces dossiers et à leurs identificateurs d’entrée, voir [Ouverture d’un dossier de la boutique de messages.](opening-a-message-store-folder.md) 
