...
1. <span data-ttu-id="9acf2-111">Incrémente le décompte de références sur l’objet de prise en charge qui est passé comme paramètre d’entrée en appelant la méthode [IUnknown::AddRef](http://msdn.microsoft.com/fr-fr/library/ms691379%28v=VS.85%29.aspx) .</span><span class="sxs-lookup"><span data-stu-id="9acf2-111">Increment the reference count on the support object that is passed as an input parameter by calling its [IUnknown::AddRef](http://msdn.microsoft.com/fr-fr/library/ms691379%28v=VS.85%29.aspx) method.</span></span> 
