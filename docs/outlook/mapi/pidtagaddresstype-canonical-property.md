...
Ces propriétés sont des exemples de propriétés d'adresse de base communes à tous les utilisateurs de messagerie. Il spécifie le système de messagerie MAPI utilisé pour gérer un message donné.
  
Cette propriété détermine également le format de la chaîne d'adresse dans le **PR_EMAIL_ADRESS** ([PidTagEmailAddress](pidtagemailaddress-canonical-property.md)). La chaîne qu'il fournit peut contenir uniquement les caractères alphabétiques majuscules de A à Z et les nombres compris entre 0 et 9.
  
Voici des exemples valides pour la chaîne: 
