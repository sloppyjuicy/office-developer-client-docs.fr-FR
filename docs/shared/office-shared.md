---
title: Office partagé
manager: soliver
ms.date: 04/20/2016
ms.audience: Developer
localization_priority: Normal
ms.assetid: 1c4937c4-0998-4f81-a994-50d3dd54727c
description: Recherchez des procédures, des exemples de code, de la documentation sur les kits de développement logiciel et les API, des références VBA, des formations et des articles techniques portant sur le développement de solutions et la personnalisation de Office.
ms.openlocfilehash: 07ac30011bd4f890d2c4e4105d63687e8ad5a154
ms.sourcegitcommit: 9d60cd82b5413446e5bc8ace2cd689f683fb41a7
ms.translationtype: HT
ms.contentlocale: fr-FR
ms.lasthandoff: 06/15/2018
ms.locfileid: "19787980"
---
# <a name="office-shared"></a>Office partagé

Recherchez des procédures, des exemples de code, de la documentation sur les kits de développement logiciel et les API, des références VBA, des formations et des articles techniques portant sur le développement de solutions et la personnalisation de Office.
  
> [!NOTE]
> Vous voulez développer des solutions qui étendent l’expérience Office sur [plusieurs plateformes](https://docs.microsoft.com/fr-FR/office/dev/add-ins/overview/office-add-in-availability) ? Consultez le nouveau [modèle de compléments Office](https://docs.microsoft.com/fr-FR/office/dev/add-ins/overview/office-add-ins). 
  
### <a name="viewed-most"></a>Articles populaires
  
- [Référence de la bibliothèque d’objets VBA Office](https://msdn.microsoft.com/fr-FR/library/office/ff862474.aspx)
  
- [Référence de langage VBA pour Office](https://msdn.microsoft.com/fr-FR/library/office/gg264383.aspx)
  
- [Référence des API Office 365](https://msdn.microsoft.com/fr-FR/office/office365/api/api-catalog)
  
- [Office 2010](https://msdn.microsoft.com/fr-FR/library/office/cc313152%28v=office.12%29.aspx)
  
### <a name="office-365-apis"></a>API Office 365
  
- [Prise en main](https://dev.office.com/getting-started)
  
- [Vue d’ensemble de la plateforme](https://msdn.microsoft.com/fr-FR/office/office365/howto/platform-development-overview)
  
### <a name="office-add-ins"></a>Compléments Office
  
- [Prise en main](https://dev.office.com/getting-started)
  
- [Vue d’ensemble de la plateforme](https://msdn.microsoft.com/fr-FR/library/office/jj220082.aspx)
  
### <a name="sharepoint-add-ins"></a>Compléments SharePoint
  
- [Prise en main](https://dev.office.com/getting-started)
  
- [Vue d’ensemble de la plateforme](https://msdn.microsoft.com/fr-FR/library/office/fp179930.aspx)
  
### <a name="code-samples"></a>Exemples de code
  
- [Exemples de code sur dev.office.com](https://dev.office.com/codesamples)
  
- [OfficeDev sur GitHub](https://github.com/OfficeDev)
  
### <a name="support"></a>Support
  
- [Support technique Office](https://support.office.com/)
  
- [Forums des développeurs Office sur MSDN](https://social.msdn.microsoft.com/Forums/office/en-US/home?category=officedev)
  
- [Forum Office sur le dépassement de la capacité de la pile](http://stackoverflow.com/questions/tagged/ms-office)
  
- [Forum Office 365 sur le dépassement de la capacité de la pile](http://stackoverflow.com/questions/tagged/office365)
  

