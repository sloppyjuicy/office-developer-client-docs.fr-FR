...
description: Microsoft InfoPath prend en charge les compléments COM pour étendre l’expérience utilisateur lors de la modification. Bien que la prise en charge des add-ins COM a été ajoutée pour la première fois dans InfoPath, d’autres applications Office telles que Microsoft Office Word et Microsoft Office Excel ont pris en charge les modules complémentaires COM depuis Office 2000.
ms.openlocfilehash: 657d3440c149b523a4b0efff445bde035453a525
ms.sourcegitcommit: c0fae34cd3a9c75a7cffcf9ae8e417ddde07a989
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 02/12/2022
ms.locfileid: "62780336"
---
...
Microsoft InfoPath prend en charge les compléments COM pour étendre l’expérience utilisateur lors de la modification. Bien que la prise en charge des add-ins COM a été ajoutée pour la première fois dans InfoPath, d’autres applications Office telles que Microsoft Office Word et Microsoft Office Excel ont pris en charge les modules complémentaires COM depuis Office 2000.
...
|:-----|:-----|
|**OnAddInsUpdate (ByVal custom() As Variant)** <br/> |Se produit lorsqu’un add-in est chargé ou déchargé dans l’environnement. |
|**OnBeginShutdown (ByVal custom() As Variant)** <br/> |Se produit lorsque l’environnement est en cours d’arrêt. |
|**OnConnection(ByVal Application As Object, ByVal ConnectMode As ext_ConnectMode, ByVal AddInInst As Object, ByVal custom() As Variant)** <br/> |Se produit lorsqu’un add-in est chargé dans l’environnement. |
|**OnDisconnection (ByVal RemoveMode As ext_DisconnectMode, ByVal custom() As Variant)** <br/> |Se produit lorsqu’un add-in est déchargé de l’environnement. |
|**OnStartupComplete (ByVal custom() As Variant)** <br/> |Se produit lorsque le démarrage de l’environnement est terminé. |
   
## <a name="registering-com-add-ins"></a>Inscription de compl?ments COM

Toutes les applications Office, y compris InfoPath, utilisent le Registre pour lister les modules complémentaires dans la collection Add-Ins COM, pour stocker l’état de connexion et stocker les informations de chargement de démarrage ou de demande. Pour les compl?ments COM InfoPath, le nom de chaque compl?ment s’affiche sous la clé suivante :
...
|:-----|:-----|:-----|
|**FriendlyName** <br/> |**String** <br/> |Nom qui s’affiche dans la boîte de dialogue Des compl? **ments COM** et qui figure dans la page Des compl? **ments** du Centre **de trust**. |
|**Description** <br/> |**String** <br/> |Chaîne qui s’affiche lorsque le module est sélectionné dans le Centre **de confiance**. |
|**LoadBehavior** <br/> |**DWORD** <br/> |Spécifie le mode de chargement du compl?ment COM. La valeur peut être une combinaison de 0, 1, 2, 8 et 16. Pour plus d’informations, voir le tableau ci-dessous. |
...
|**Valeur**|**Description**|
|:-----|:-----|
|0  <br/> |Déconnecté. Le compl?ment s’affiche comme inactif dans la **boîte de dialogue Compl?ment COM** . |
|1  <br/> |Connecté. Le compl?ment s’affiche comme actif dans la boîte **de dialogue Compl?ment COM** . |
|2  <br/> |Charger au démarrage. Le add-in est chargé et connecté au démarrage de l’application hôte. |
|8   <br/> |Charger à la demande. Le add-in est chargé et connecté lorsque l’application hôte l’exige, par exemple lorsqu’un utilisateur clique sur un bouton qui utilise les fonctionnalités du module. |
|16  <br/> |Connecter la première fois. Le add-in est chargé et connecté la première fois que l’utilisateur exécute l’application hôte après avoir inscrit le module. |
...
3. Dans le **Project Types** de la boîte de dialogue Nouveau **Project**, cliquez sur le dossier Autres **types** de projets, puis sur **Extensibilité**.
    
4. Dans le **volet Modèles** , cliquez sur **Ajouter partagé**.
    
5. Dans la **zone** Nom, tapez un nom pour le projet. 
    
6. Dans la **zone Emplacement** , tapez un chemin d’accès au dossier ou cliquez sur **Parcourir** et sélectionnez un chemin d’accès de dossier, puis cliquez sur **OK**. **L’Assistant De ment partagé s’affiche**. 
...
1. Cliquez avec le bouton droit sur le nœud racine du projet d’installation dans l’Explorateur de **solutions, cliquez** sur **Afficher**, puis Sur **Éditeur, puis** sur **Registre**.
    
2. Dans le volet gauche, cliquez sur le plus pour développer **HKEY_LOCAL_MACHINE,** **Logiciel**, **Microsoft**, **InfoPath**, puis **AddIns**.
...
> [!NOTE]
> Si le .NET Framework ou d’autres assemblys requis du projet d’installation ne sont pas déjà installés sur les ordinateurs cibles, il se peut que le fichier .msi ne s’installe pas correctement. En outre, vous ne pouvez pas distribuer le .msi puis essayer d’installer le .msi fichier. Vous devez également distribuer les autres fichiers de support dans le même dossier que le fichier .msi d’origine généré par Visual Studio. 
...
|**Event**|**Description**|
|:-----|:-----|
|[NewXDocument](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.NewXDocument.aspx) Événement  <br/> |Se produit lorsqu'un nouveau formulaire est créé. |
|[Quit](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.Quit.aspx) Événement  <br/> |Se produit lorsque l’utilisateur quitte InfoPath. |
|[WindowActivate](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.WindowActivate.aspx) Événement  <br/> |Se produit lors de l'activation d'une fenêtre de document. |
|[WindowDeactivate](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.WindowDeactivate.aspx) Événement  <br/> |Se produit lors de la désactivation d'une fenêtre de document. |
|[WindowSize](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.WindowSize.aspx) Événement  <br/> |Se produit lorsqu’une fenêtre de document est redimensionnée ou déplacée. |
|[XDocumentBeforeClose](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.XDocumentBeforeClose.aspx) Événement  <br/> |Se produit immédiatement avant la fermeture d'un document. |
|[XDocumentBeforePrint](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.XDocumentBeforePrint.aspx) Événement  <br/> |Se produit juste avant l’impression d’un des documents ouverts. |
|[XDocumentBeforeSave](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.XDocumentBeforeSave.aspx) Événement  <br/> |Se produit juste avant l’enregistrement d’un des documents ouverts. |
|[XDocumentChange](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.XDocumentChange.aspx) Événement  <br/> |Se produit lorsqu’un formulaire est créé, lorsqu’un formulaire est ouvert ou lorsqu’un autre formulaire est activé. |
|[XDocumentOpen](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath._ApplicationEvents_Event.XDocumentOpen.aspx) Événement  <br/> |Se produit à l'ouverture d'un document. |
   
Pour capturer ces événements dans le compl?ment COM, vous devez d déclarer les variables de niveau de classe suivantes dans **votre classe Connecter** suivante : 
...
- [Créer un Office COM à l’aide de Visual C# .NET](https://support.microsoft.com/help/302901/how-to-build-an-office-com-add-in-by-using-visual-c-net)
