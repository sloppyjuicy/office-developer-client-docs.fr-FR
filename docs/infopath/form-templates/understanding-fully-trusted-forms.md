...
ms.openlocfilehash: 0b92326b7d2ac2543e3e34a7dac985cdc93c5298
...
5. Dans le dossier dans lequel vous avez extrait les fichiers de formulaire, ouvrez le fichier de définition de formulaire (.xsf), `manifest.xsf` nommé par défaut, dans un éditeur de texte tel que Microsoft Bloc-notes.
...
6. Ajoutez les attributs suivants à l'élément **xDocumentClass** du fichier .xsf :
   ...
   > [!NOTE]
   > [!REMARQUE] Les valeurs utilisées pour le nom URN peuvent être une valeur de chaîne de quelconque sorte, du moment que cette valeur est unique. Il doit y avoir au moins deux valeurs après le `urn:` préfixe, et ces valeurs doivent être séparées par un deux-points. En outre, le nom URN ne doit pas dépasser 255 caractères.
  
7. Enregistrez et fermez le fichier .xsf, puis ouvrez le fichier de modèle XML (.xml) `Template.xml` nommé par défaut, dans un éditeur de texte tel que Bloc-notes.

8. Supprimez **l’attribut href** de `mso-infoPathSolution` l’instruction de traitement et remplacez-le par le même attribut de nom que celui que vous avez utilisé à l’étape 6 pour le fichier .xsf.
