...
2. **Configurer un ordinateur de développement.**
    ...
    3. Pour plus d’informations sur la compilation et l’utilisation de l’assembly de proxy ProjectServerServices.dll pour les services WCF dans l’interface PSI, voir Utilisation d’un [assembly de proxy PSI et descriptions IntelliSense suivantes](#pj15_PrerequisitesWCF_BuildingProxy).
...
3. Dans la **boîte de dialogue Ajouter** une référence de service, dans la zone de texte Adresse, <https://localhost:32843/> tapez _GUID_/psi/_ServiceName.svc_, puis appuyez sur **Entrée**. Remplacez _le GUID_ par le nom du répertoire virtuel de l’application de service Project Server, tel que 534c37eb00d74ccfadcecf9827e95239. _Remplacez ServiceName_ par le nom du service, tel que Resource (voir figure 3).
   ...
   Les URL, telles que `https://localhost:32843/534c37eb00d74ccfadcecf9827e95239/PSI/Resource.svc` les URL, sont standard pour Project services serveur.
...
La configuration du service PSI n’utilise pas les liaisons et points de terminaison par défaut. Project Server requiert que les applications accèdent aux services PSI via le projectserver.svc frontal, qui agit comme routeur pour les appels aux services frontaux. Pour créer le fichier app.config, faites les étapes suivantes :
  
1. Si vous définissez une référence à l’assembly de proxy ProjectServerServices.dll ou si vous ajoutez le fichier proxy source pour un service, l’application ne contient pas de app.config fichier. Ajoutez un nouvel élément au Visual Studio projet. Dans la **boîte de dialogue Ajouter un nouvel** élément, choisissez le modèle de fichier de **configuration de l’application** , nommez-le app.config puis choisissez **Ajouter**.
...
>
> Le correctif d’une application ASMX est différent. Pour plus d’informations, voir la section Utilisation _de l’authentification_ multiple dans [Prerequisites for ASMX-based code samples in Project](prerequisites-for-asmx-based-code-samples-in-project.md).
...
Voici quelques liens sur la page Serveur Paramètres à utiliser pour les activités de nettoyage classiques après l’exécution d’exemples de code :
