...
- **Microsoft.Office.Project.Server.DataServices** Les classes et les membres de cet espace de noms sont utilisés en interne par le service **OData** dans Project Online pour accéder aux tables de rapports de la base de données Project. Les classes **DataServices** ne sont pas documentées.
...
- **Microsoft.ProjectServer.Client.dll** Cet assemblage contient le CSOM pour le développement avec le .NET Framework 4. L’assembly se trouve dans `%ProgramFiles%\Common Files\Microsoft Shared\Web Server Extensions\15\ISAPI\Microsoft.ProjectServer.Client.dll`. Le développement des applications avec l’espace de noms **Microsoft.ProjectServer.Client** est indépendant des APIs et les services Project Server en local, bien que les applications peuvent fonctionner avec soit une installation en local ou en ligne du Project Server. Pour les assemblages CSOM associés qui peuvent être utilisés pour Windows Phone 8, Microsoft Silverlight, ou JavaScript avec des applications web, Consultez [Microsoft.ProjectServer.Client](https://msdn.microsoft.com/library/Microsoft.ProjectServer.Client.aspx).

- **Microsoft.Office.Project.Server.Schema.dll** Le Kit de développement logiciel (SDK) Project 2013 ne documente pas l’espace de noms **Microsoft.Office.Project.Server.Schema** , qui se trouve dans l’assembly `[Windows]\Microsoft.NET\assembly\GAC_MSIL\Microsoft.Office.Project.Schema\v4.0_15.0.0.0__71e9bce111e9429c\Microsoft.Office.Project.Schema.dll` . L’espace de noms contient les définitions de toutes les catégories **DataSet**, **table de données**, et **DataRow** utilisées dans la PSI, ainsi que de nombreuses autres catégories similaires utilisent Project Server en interne. Les catégories publiques dans chaque service PSI sont présentées dans la référence de service spécifique. Par exemple, la catégorie **DriverDataSet.DriverRow** est présentée dans l’espace de noms [WebSvcDriver](https://msdn.microsoft.com/library/WebSvcDriver.aspx).
...
- [CustomFields](https://msdn.microsoft.com/library/WebSvcCustomFields.CustomFields.aspx) gère les champs personnalisés d’entreprise. Inclut les méthodes d’extraction et d’archivage, ainsi que les méthodes CRUD (Créer, Lire, Mettre à jour et Supprimer) pour les champs personnalisés d’entreprise.
...
- [Events](https://msdn.microsoft.com/library/WebSvcEvents.Events.aspx) gère les associations de gestionnaires d’événements Project Server. Inclut les méthodes CRUD pour les associations de gestionnaires d’événements Project Server pour un événement spécifique ou pour toutes les associations de gestionnaires d’événements.
...
> [!CAUTION]
> Le service **LoginWindows** n’est pas utilisé dans les applications WCF, ou pour les applications qui s’exécutent sur des installations Project Server qui utilisent uniquement l’authentification par revendications ou **OAuth**; dans ce cas, la méthode **Login** retourne toujours **false**. L’authentification des déclarations gère l’authentification Windows intégrée.
...
- [Resource](https://msdn.microsoft.com/library/WebSvcResource.Resource.aspx) gère les ressources d’entreprise. Extrait, archive, met à jour ou crée des ressources ou des utilisateurs Project Server et leurs paramètres d’autorisation ; recherche des ressources par nom ou GUID ; lit les données des ressources ou des utilisateurs, ainsi que la structure de répartition des ressources (RBS) et les informations de sécurité associées ; obtient toutes les affectations d’une ressource ; et réinitialise les mots de passe utilisateur. La classe **Resource** inclut des méthodes CRUD pour les délégations d’utilisateurs.
- [ResourcePlan](https://msdn.microsoft.com/library/WebSvcResourcePlan.ResourcePlan.aspx) gère les plans de ressources. Extrait, archive, publie et inclut les méthodes CRUD pour les plans de ressources.
