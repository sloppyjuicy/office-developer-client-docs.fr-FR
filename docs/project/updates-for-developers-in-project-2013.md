...
Project Server 2013 s’appuie sur l’infrastructure introduite avec Microsoft Office Project Server 2007 et étendue par Project Server 2010. Project Server 2013 ajoute un modèle objet côté client (CSOM) qui est refactorisant et simplifié à partir de l’interface PSI (Project Server Interface), et inclut une bibliothèque JavaScript et des bibliothèques .NET Framework 4 pour les applications Windows, Windows Phone 8 et Microsoft Silverlight. Le CSOM est conçu pour le développement Project Online et fonctionne également avec une installation Project Server en local. 

Les bases de données Project Server sont combinées en une seule base de données ; vous pouvez donc accéder aux vues et tables de rapports en ligne via un service OData. Le modèle objet client et le service OData incluent une interface REST. Project Les flux de travail serveur peuvent être créés à l’aide SharePoint Designer 2013. Project Professionnel 2013 peut s’intégrer aux données de rapports Project Server, aux listes de tâches SharePoint et à d’autres contenus externes à l’aide du modèle d’extensibilité des Office Pour les volets De tâches. Project Standard 2013 peut utiliser des add-ins du volet Des tâches pour intégrer du contenu externe général.
...
> [!NOTE]
> Project Server 2013 et Project Online n’utilisent pas le modèle d’authentification d’application uniquement dans SharePoint 2013 (voir types de stratégie d’autorisation des SharePoint [2013](https://msdn.microsoft.com/library/124879c7-a746-4c10-96a7-da76ad5327f0%28Office.15%29.aspx)). 
  
Pour plus d’informations sur le développement, la distribution, l’hébergement et la gestion des add-ins, voir [SharePoint Add-ins](https://docs.microsoft.com/sharepoint/dev/sp-add-ins/sharepoint-add-ins) and [Office Add-ins](https://docs.microsoft.com/office/dev/add-ins/overview/office-add-ins), et des rubriques connexes dans la documentation du développeur SharePoint Server 2013 et Office 2013. Pour plus d’informations sur l’étendue de demande d’autorisation pour SharePoint d’autres SharePoint, voir Autorisations de SharePoint [2013](https://msdn.microsoft.com/library/5f7a8440-3c09-4cf8-83ec-c236bfa2d6c4%28Office.15%29.aspx).
...
Pour plus d’informations sur le développement avec le modèle CSOM, voir modèle objet [côté client (CSOM) pour Project 2013](client-side-object-model-csom-for-project-2013.md). Pour plus d’informations sur l’interface REST dans SharePoint applications, voir Programmation à l’aide du *service REST SharePoint* dans la documentation SharePoint 2013 pour les développeurs. 
...
> [!IMPORTANT]
> L’accès direct n’est pas pris en charge pour les vues et tables provisoires (préfixes), publiées ( ) et `draft` `pub` archivées ( `ver` ). Les rapports doivent utiliser uniquement les tables et vues de rapports, qui ont `dbo` le préfixe. Par exemple, le dbo. MSP_EpmProject table inclut la liste des projets dans l’instance Project Web App instance. 
>
...
La figure 2 illustre le complément du volet de tâches **Hello ProjectData** qui interroge le service **ProjectData**, puis compare les données du projet actuel avec les moyennes de tous les projets. Le Project SDK 2013 inclut le code source complet du module. 
...
> [!NOTE]
> Project Standard 2013 ne peut pas s’intégrer directement à Project Server 2013 via des modules de volet de tâches. 
...
Il peut y avoir plusieurs serveurs Project Web App (également appelés serveurs web frontux ou WPF) dans une batterie de serveurs SharePoint qui inclut l’application de service Project frontale. Les récepteurs d’événements peuvent également être appelés gestionnaires d’événements. Les gestionnaires d’événements locaux peuvent être implémentés avec le code de confiance totale et déployés sur l’ensemble des WFE pour une installation Project Server locale. Les récepteurs d’événements à distance peuvent être implémentés dans les services web sur des serveurs locaux ou à distance et accessibles par l’intermédiaire de plusieurs WFE et installations de Project Server. Project Online pouvez utiliser uniquement des récepteurs d’événements distants.
  
Project Les serveurs de gestion d’événements sont gérés par SharePoint pour chaque instance Project Web App, plutôt que par une page Project Web App Paramètres spécifique. In the SharePoint Central Administration application, choose **General Application Paramètres,** choose **Manage** under **PWA Paramètres**, and then choose the instance in the **Project Web App Instance** drop-down list on the PWA Paramètres page. Pour ajouter un gestionnaire d’événements local ou un récepteur d’événements à distance, choisissez **Gestionnaires d’événements côté serveur**.
...
Si vous avez besoin d’accéder aux jeux de données PSI pour un responsable d’événements local, vous pouvez copier l’assembly Microsoft.Office.Project.Schema.dll à partir de [Windows]\Microsoft.NET\assembly\GAC \_ MSIL\Microsoft.Office. Project. Schéma\v4.0_15.0.0.0__71e9bce111e9429c. 
...
- **Les scénarios** [d’extension PSI](https://docs.microsoft.com/previous-versions/office/developer/office-2010/ff843378(v=office.14)) des extensions sont supprimés et ne seront pas pris en charge dans les prochaines publication. Ces scénarios locaux Project Server 2013 permettaient l’intégration à l’aide Windows services WCF (Custom Windows Communication Foundation). 
...
- **Interface ASMX pour l’interface PSI** L’interface PSI inclut des interfaces en double pour le développement d’extensions Project server en local. L’interface des services web ASMX a été introduite avec la première implémentation de l’interface PSI dans Office Project Server 2007. Project Server 2010 a ajouté l’interface des services WCF, où le modèle objet duplique essentiellement les services web ASMX. Bien Project Server 2013 continue de prendre en charge ASMX et WCF, les nouvelles solutions qui nécessitent l’interface PSI doivent utiliser les services WCF. Si possible, les nouvelles solutions doivent être écrites en utilisant le modèle objet client. 
  
  Les services web ASMX psi sont supprimés dans Project Server 2013. Pour qu’elles fonctionnent dans les futures versions de Project Server, les solutions utilisant les services web ASMX doivent être réécrites afin d’utiliser soit les services WCF, soit le modèle objet client. Pour plus d’informations, voir la section Mise à niveau des applications avec *les API Project Server* dans Project Server [programmabilité.](project-server-programmability.md)
...
Office Les add-ins offrent une nouvelle solution pour vendre des solutions et intégrer Project Standard 2013 au contenu web et à d’autres produits Office 2013. Vous pouvez également créer de nouvelles façons d’intégrer Project Professionnel 2013 avec les données Project Server et les listes SharePoint par le biais de Office de Office de tâches.
