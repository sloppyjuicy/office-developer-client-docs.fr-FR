...
ms.openlocfilehash: cb50c074ca48be12ef2f34246af2abe3436b671d
...
Les versions précédentes des fichiers Visio enregistrés dans un format de fichier binaire propriétaire (.vsd) ou un format de fichier de dessin XML Visio à document unique (.vdx). Visio 2013 introduit un nouveau format de fichier (.vsdx), basé sur les technologies d’archivage XML et ZIP. Comme dans les versions précédentes de Visio, les fichiers sont enregistrés dans un seul conteneur. Toutefois, contrairement aux fichiers hérités, le nouveau format de fichier peut être ouvert, lu, mis à jour, modifié et construit sans automatiser l’application Visio 2013. Les développeurs qui sont familiarisés avec la manipulation du code XML ou l’utilisation de l’espace de noms [System.IO.Packaging](/dotnet/api/system.io.packaging?view=netframework-4.8) peuvent rapidement commencer à utiliser le nouveau format de fichier par programmation. Les développeurs qui ont travaillé avec le format de dessin XML Visio des versions précédentes peuvent constater que la plupart des structures de ce format ont été conservées dans le nouveau format de fichier.
...
|[OPC : une nouvelle norme pour les packages de données](/archive/msdn-magazine/2007/august/opc-a-new-standard-for-packaging-your-data) <br/> |Cet article de MSDN Magazine décrit les spécifications des normes Open Packaging Conventions sous forme de concept. |
|[Éléments fondamentaux des OPC (Open Packaging Conventions)](/previous-versions/office/office-12/ee361919(v=office.12)) <br/> [Présentation des formats de fichier Office (2007) Open XML](/previous-versions/office/developer/office-2007/aa338205(v=office.12)) <br/> |Ces deux articles traitent de la manière dont les spécifications des normes Open Packaging Conventions ont été appliquées aux fichiers Microsoft Office. Ils contiennent des descriptions sur le fonctionnement des relations dans un package, et incluent également quelques exemples de code. |
...
Avant de pouvoir manipuler les données du fichier, vous devez d’abord ouvrir le fichier dans un objet [Package](/dotnet/api/system.io.packaging.package?view=netframework-4.8), qui se trouve dans l’espace de noms [System.IO.Packaging](https://docs.microsoft.com/dotnet/api/system.io.packaging?view=netframework-4.8). L’objet **Package** représente le fichier Visio dans son ensemble. Il expose les membres qui vous permettent de sélectionner des parties de document individuelles dans le package de fichiers. En particulier, la classe **Package** expose la méthode statique [Open(String, FileMode, FileAccess)](/dotnet/api/system.io.packaging.package.open?view=netframework-4.8) que vous utilisez pour ouvrir un fichier en tant que package. Il expose également une méthode [Close()](/dotnet/api/system.io.packaging.package.close?view=netframework-4.8)pour fermer le package une fois que vous avez terminé.
  
> [!TIP]
> Il est recommandé d’utiliser un **à l’aide de** bloc pour ouvrir le fichier Visio dans l’objet **Package** afin de ne pas avoir à fermer explicitement le package de fichiers lorsque vous avez terminé. Vous pouvez également appeler explicitement la méthode **Package.Close** dans le **enfin** bloc d’une construction **try/catch/finally**.
  
Utilisez le code suivant pour obtenir le chemin d’accès complet du fichier « Visio Package.vsdx » à l’aide d’un objet [FileInfo](/dotnet/api/system.io.fileinfo?view=netframework-4.8), transmettez le chemin d’accès comme un argument vers la méthode **Package.Open**, puis renvoyez un objet **Package** au code appelant.
...
Une fois que le fichier Visio 2013 est ouvert en tant que package, vous pouvez accéder aux parties de document qu’il contient à l’aide de la classe [PackagePart](https://msdn.microsoft.com/library/System.IO.Packaging.PackagePart.aspx) incluse dans l’espace de noms **System.IO.Packaging**. Les objets **PackagePart** peuvent être instanciés individuellement ou en tant que collection. La classe **Package** expose une méthode [GetParts()](https://msdn.microsoft.com/library/System.IO.Packaging.Package.GetParts.aspx) et une méthode [GetPart(Uri)](https://msdn.microsoft.com/library/System.IO.Packaging.Package.GetPart.aspx) pour extraire des objets **PackagePart** du **Package**. La méthode **Package.GetParts** retourne une instance de la classe [PackagePartCollection](https://msdn.microsoft.com/library/System.IO.Packaging.PackagePartCollection.aspx), avec laquelle vous pouvez ensuite interagir comme toute autre collection qui implémente l’interface [IEnumerator\<T\>](/dotnet/api/system.collections.generic.ienumerator-1?redirectedfrom=MSDN&view=netframework-4.7.2).
...
> [!NOTE]
> Vous pouvez également obtenir une référence à un **PackagePart** spécifique à l’aide de la méthode **Package.GetPart** et de l’URI du **PackagePart**, sans passer par l’étape d’obtention des relations du composant de package. Toutefois, certains composants de package dans le package de fichiers Visio peuvent être enregistrés à un emplacement différent de leur emplacement par défaut dans un package. Vous ne pouvez pas partir du principe qu’un composant de package se trouve toujours dans le même URI pour chaque fichier. Il est préférable d'utiliser les relations pour accéder aux objets **PackagePart** individuels.
...
Comme mentionné précédemment, vous pouvez également obtenir les objets **PackagePart** à l’aide de leur relation avec d’autres objets **PackagePart**. Cela est important car, pour un document Visio de toute complexité, la plupart des objets **PackagePart** n’ont pas de relation avec le **Package**. Par exemple, un composant Contenu de page individuel dans le package de fichiers (autrement dit, /visio/pages/page1.xml) a une relation avec la partie Index de page (autrement dit, /visio/pages/pages.xml), mais pas avec le package de fichiers lui-même. Si vous n’avez pas l’URI exact de la page individuelle dans le package, vous pouvez utiliser sa relation avec le composant Index de page pour obtenir une référence à celle-ci.
  
La classe **PackagePart** présente une méthode [M:System.IO.Packaging.PackagePart.GetRelationshipsByType(System.String)](https://msdn.microsoft.com/library/System.IO.Packaging.PackagePart.GetRelationshipsByType.aspx) qui vous permet de renvoyer un objet **PackageRelationshipCollection** contenant uniquement un seul type d’objet **PackageRelationship**. Une fois que vous avez l’objet **PackageRelationshipCollection**, vous pouvez sélectionner l’objet **PackageRelationship** nécessaire dans la collection, puis référencer l’objet **PackagePart**.
...
Une fois que vous avez obtenu une référence à un objet **XElement** contenu dans un objet **XDocument**, vous pouvez le manipuler comme n’importe quelle autre donnée XML et, par conséquent, modifier les données contenues dans le fichier Visio. Par exemple, si une forme a du texte lorsqu’elle est ouverte dans Visio, l’élément **Shape** correspondant contient au moins un élément **Text**. Si vous modifiez la valeur de cet élément **Text**, le texte de la forme est modifié lorsque le fichier est affiché dans Visio.
...
Le code suivant utilise la classe [XmlWriter](https://msdn.microsoft.com/library/System.Xml.XmlWriter.aspx) et classe [XmlWriterSettings](https://msdn.microsoft.com/library/System.Xml.XmlWriterSettings.aspx) pour réécrire le code XML dans la partie de package. Bien que vous puissiez utiliser la méthode [Save()](https://msdn.microsoft.com/library/System.Xml.Linq.XDocument.Save.aspx) pour enregistrer le code XML dans le composant, les classes **XmlWriter** et **XmlWriterSettings** vous permettent de contrôler plus précisément la sortie, notamment en spécifiant le type d’encodage. La classe **XDocument** expose une méthode [WriteTo(XmlWriter)](https://msdn.microsoft.com/library/System.Xml.Linq.XDocument.WriteTo.aspx) qui prend un objet **XmlWriter** et réécrit du code XML dans un flux.
...
Certaines modifications apportées aux données d’un fichier peuvent nécessiter que Visio recalcule le document lorsqu’il ouvre le fichier. Visio fournit beaucoup de logique pour un diagramme, en particulier pour les relations de forme (c’est-à-dit, quand une forme dépend d’une autre) et la connexion de formes. Si l’une des données sur lesquelles repose la logique personnalisée est modifiée, Visio doit propager les modifications à toutes les données calculées affectées dans le fichier.
  
Le format de fichier Visio 2013 comprend quelques techniques que vous pouvez utiliser pour recalculer des données dans le fichier. Il existe trois types de scénarios que vous devez prendre en compte lorsque vous décidez si vous devez recalculer le fichier Visio et comment le faire :
  
- Les modifications apportées aux données n’affectent aucune autre valeur au format de fichier. Vous n’avez pas besoin d’ajouter d’instructions supplémentaires à Visio pour recalculer le document. Comme indiqué précédemment, vous pouvez souvent modifier le texte d’une forme sans avoir à recalculer le document.

- Les modifications apportées aux données sont limitées à la modification des valeurs des cellules ShapeSheet dans le XML, et d’autres valeurs ShapeSheet dépendent de ces données. Dans ce cas, vous devez ajouter une instruction de traitement XML (à l’aide de la classe [XProcessingInstruction](https://msdn.microsoft.com/library/System.Xml.Linq.XProcessingInstruction.aspx)) à l’élément **Cell** qui a été modifié. Par exemple, la cellule **ThemeIndex** d’une forme affecte les valeurs de plusieurs autres cellules ShapeSheet contenues dans la forme. Si vous modifiez la cellule **ThemeIndex** dans le fichier lui-même (par exemple, l’élément **Cell** avec une valeur **N** « ThemeIndex »), vous devez ajouter une instruction de traitement à l’élément **Cell** afin que les valeurs dépendantes soient mises à jour.

- Les modifications apportées aux données affectent l’emplacement d’un connecteur ou de points de connexion. Une autre situation se produit lorsque de nombreuses modifications sont apportées aux données ShapeSheet et que vous souhaitez recalculer l’ensemble du document avec une instruction (plutôt que d’ajouter des instructions de traitement individuelles pour chaque modification). Dans ce cas, vous pouvez demander à Visio de recalculer l’intégralité du document lorsqu’il est ouvert. Pour ce faire, ajoutez une propriété **RecalcDocument** à la partie Propriétés du fichier personnalisé (/docProps/custom.xml) du package Visio. L’ajustement de la position ou de la taille des formes dans un diagramme connecté est un exemple de ce type de scénario.
...
La couleur de la forme repose sur la valeur de la cellule **ThemeIndex**, elle détermine le thème actif dont la forme hérite. Dans l’exemple précédent, la forme est définie pour hériter d’un autre thème (la cellule **ThemeIndex** est définie sur une valeur de 25). Si vous n’utilisez pas une instruction de traitement, la couleur du texte de la forme, qui est également affectée par la cellule **ThemeIndex**, n’est pas recalculée. La couleur de remplissage de la forme devient blanc, mais étant donné que son texte reste blanc, le texte est illisible. En outre, sans l’instruction de traitement, Visio risque de mettre à jour la forme ultérieurement, ce qui place le fichier dans un état instable, dans lequel les valeurs de mise en forme de la forme peuvent être mises à jour de manière imprévisible.
...
Utilisez le code suivant pour modifier la valeur de la cellule **PinY** de la forme Start/End par rapport aux exemples précédents. Le code sélectionne l’élément **Cell** qui contient les données de cellule **PinY** en tant qu’objet **XElement** en utilisant la valeur de son attribut **N**. Le code ajoute ensuite une instruction de recalcul à la partie Propriétés du fichier personnalisé du fichier Visio.
...
L’un des scénarios les plus courants pour modifier un package de fichiers est l’ajout d’un nouveau composant de document au package. Par exemple, si vous souhaitez ajouter une page au dessin Visio en ajoutant du contenu au package, vous devez ajouter un composant Contenu de page au package.
...
Utilisez la procédure suivante pour créer une partie d’extensibilité du ruban dans le fichier Visio. Le nouveau composant d’extensibilité du ruban ajoute au ruban un nouvel onglet avec un seul groupe qui contient un seul bouton.
...
Nous aimerions reconnaître la contribution et l’entrée du MVP Visio **Al Ed** dans la création des exemples de code contenus dans cet article technique. Al est un expert reconnu dans la manipulation du format de fichier Visio, notamment le format de dessin XML Visio (.vdx) et le nouveau format de fichier Visio (.vsdx). Al a créé des projets qui explorent les formats de fichier Visio par programmation et exposent les structures à l’intérieur.
...
- [Manipuler des documents au format Open Office XML](/previous-versions/office/developer/office-2007/aa982683(v=office.12))

- [Créer un document avec des espaces de noms (C#) (LINQ vers XML)](/previous-versions/bb387075(v=vs.140))

- [Ajouter des composants XML personnalisés à des documents sans démarrer Microsoft Office](/previous-versions/visualstudio/visual-studio-2008/bb608597(v=vs.90))
