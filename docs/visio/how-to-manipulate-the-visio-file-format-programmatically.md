...
Avant de pouvoir manipuler les données du fichier, vous devez d’abord ouvrir le fichier dans un objet [Package](/dotnet/api/system.io.packaging.package?view=netframework-4.8&preserve-view=true), qui se trouve dans l’espace de noms [System.IO.Packaging](/dotnet/api/system.io.packaging?view=netframework-4.8&preserve-view=true). L’objet **Package** représente le fichier Visio dans son ensemble. Il expose les membres qui vous permettent de sélectionner des parties de document individuelles dans le package de fichiers. En particulier, la classe **Package** expose la méthode statique [Open(String, FileMode, FileAccess)](/dotnet/api/system.io.packaging.package.open?view=netframework-4.8&preserve-view=true) que vous utilisez pour ouvrir un fichier en tant que package. Il expose également une méthode [Close()](/dotnet/api/system.io.packaging.package.close?view=netframework-4.8&preserve-view=true)pour fermer le package une fois que vous avez terminé.
...
Utilisez le code suivant pour obtenir le chemin d’accès complet du fichier « Visio Package.vsdx » à l’aide d’un objet [FileInfo](/dotnet/api/system.io.fileinfo?view=netframework-4.8&preserve-view=true), transmettez le chemin d’accès comme un argument vers la méthode **Package.Open**, puis renvoyez un objet **Package** au code appelant.
