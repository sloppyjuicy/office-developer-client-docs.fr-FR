---
title: Problèmes connus dans le développement XLL Excel
manager: soliver
ms.date: 11/16/2014
...
---
# <a name="known-issues-in-excel-xll-development"></a>Problèmes connus dans le développement XLL Excel
...
Lorsqu’un XLL inscrit une fonction ou une commande, Excel crée un nouveau nom pour la ressource et associe une référence à la fonction DLL avec ce nom. Le nom est tiré du quatrième argument, *pxFunctionText* , de la [fonction xlfRegister.](xlfregister-form-1.md) Ce nom est masqué dans les boîtes de dialogue normales pour la gestion des noms de feuille de calcul. Pour désinsister une fonction ou une commande, vous devez supprimer le nom à l’aide de la fonction [xlfSetName,](xlfsetname.md) en passant le nom sans passer de définition. Toutefois, un bogue empêche cette suppression du nom des listes de l’Assistant Fonction. 
...
Excel autorise l’inscription des fonctions qui retournent une **XLOPER** ou **XLOPER12** en modifiant un argument en place. Toutefois, si un argument /  **XLOPER XLOPER12** pointe vers la mémoire, et que le pointeur est ensuite remplacé par la valeur de retour de la fonction DLL, Excel peut fuiter de la mémoire. Excel n’affiche pas d’erreur, mais il peut finir par se crasher. En outre, si la DLL a alloué de la mémoire pour la valeur de retour, Excel peut essayer de libérer cette mémoire, ce qui peut provoquer un arrêt immédiat de l’application. Par conséquent, vous ne devez pas modifier les /  arguments **XLOPER XLOPER12** en place. Tous les arguments **XLOPER** ou **XLOPER12** doivent être traités comme étant strictement en lecture seule. 
