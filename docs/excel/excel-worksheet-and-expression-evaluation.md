...
ms.openlocfilehash: d78511e263c0f878492958c937643079e5af95ad
ms.sourcegitcommit: 193df57ebf141020852d2ebc8cf0931edb71574a
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 01/25/2022
ms.locfileid: "62199590"
---
...
Toutes les chaînes que l’utilisateur voit Excel ont été stockées en interne en tant que chaînes Unicode depuis de nombreuses versions. Les chaînes de feuille de calcul Unicode peuvent comporter jusqu’à 32 767 caractères<sup>(2 15</sup> - 1) et contenir n’importe quel caractère Unicode valide.
  
Lors de la premi�re introduction de l�API�C, les cha�nes de feuille de calcul �taient des cha�nes d�octets limit�es � une longueur de 255�caract�res, et l�API�C refl�tait ces limitations. Avec Excel 2007, l’API C est mise à jour pour gérer Excel chaînes Unicode longues. Cela signifie que les fonctions DLL inscrites de la bonne manière peuvent accepter des arguments Unicode et renvoyer des chaînes Unicode.
...
Excel renvoie l�erreur **#VALUE!** si un argument de fonction ou d�op�rateur ne peut pas �tre converti en type requis. Dans le cas d’arguments de fonction qui ne peuvent pas être convertis, par `=LN("X")` exemple, Excel n’appelle pas le code de fonction. Il s�agit d�un point important � retenir lors de l��criture et du d�bogage de vos propres fonctions de compl�ment.
  
Certaines fonctions retournent cette erreur si un argument ne peut pas être converti dans le code de la fonction. Par exemple, `DATEVALUE("30-Feb-2007")` échoue avec cette erreur, même si l’argument est du bon type. Dans ce cas, c’est la fonction qui retourne l’erreur à partir de son code. Certaines fonctions renvoient cette erreur même si les types et plages de valeurs sont permis, par exemple renvoie `FIND("a","xyz")` cette erreur.
...
Excel g�n�re l�erreur **#REF!** au sein d�une expression lorsqu�elle est copi�e vers un emplacement o� la r�f�rence relative obtenue sort des limites. Par exemple, si la cellule B2 contient la formule, le fait de la copier dans la cellule B1 entraîne une formule `=A1` **=#REF!**. Cette erreur est �galement g�n�r�e dans les formules contenant une r�f�rence qui est remplac�e dans une op�ration de couper-coller ou qui est supprim�e lors de la suppression d�une ligne, d�une colonne ou d�une feuille de calcul. Certaines fonctions qui peuvent renvoyer des références peuvent renvoyer cette erreur, par exemple, `OFFSET(A1,-1,-1)` . Les noms de feuille de calcul dont les d�finitions contiennent des r�f�rences qui deviennent non valides se voient attribuer cette erreur.
...
De nombreuses fonctions num�riques et math�matiques int�gr�es dans Excel renvoient l�erreur **#NUM!** lorsqu’une entrée numérique se trouve en dehors de la plage autorisée, par exemple, `LN(0)` . Vous devez envisager de renvoyer cette erreur à partir de votre fonction de complément pour indiquer qu’une entrée numérique est non valide ou hors de la plage.
