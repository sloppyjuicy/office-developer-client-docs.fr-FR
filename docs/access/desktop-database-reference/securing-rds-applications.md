...
<<<<<<< Tête RDS peut communiquer avec un serveur Web IIS en cours d’exécution dans l’un des trois modes d’authentification de mot de passe : anonyme, de base, ou l’authentification stimulation/réponse NT (appelée authentification Windows intégrée dans Windows 2000). Ces paramètres déterminent la façon dont un serveur Web contrôle l'accès (par exemple, nécessité pour un ordinateur client de disposer de privilèges d'accès explicites sur le serveur Web Windows NT).
