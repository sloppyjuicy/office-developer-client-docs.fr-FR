...
Définit ou renvoie une valeur qui donne des informations sur une table liée. Type de données **String** en lecture/écriture.
...
La valeur de la propriété **Connect** est un type de données **String** composé d'un spécificateur de type de base de données et de zéro caractères ou plus séparés par des points-virgules. La propriété **Connect** transmet des informations supplémentaires à ODBC et certains pilotes ISAM si nécessaire.

Pour un objet **TableDef** qui représente une table liée, la valeur de la propriété **Connect** est constituée d'une ou de deux parties (un spécificateur de type de base de données et un chemin à la base de données), chacune se terminant par un point-virgule.
