...
En *mode de mise à jour immédiate* (dans lequel le fournisseur écrit les modifications apportées à la source de données sous-jacente après l'appel de la méthode [Update](update-method-ado.md) ), la propriété **OriginalValue** renvoie la valeur de champ qui existait avant toute modification (c'est-à-dire, étant donné que le dernier appel de la méthode **Update** ). C’est la même valeur que la méthode [CancelUpdate](cancelupdate-method-ado.md) utilise pour remplacer la propriété [Value](value-property-ado.md).
