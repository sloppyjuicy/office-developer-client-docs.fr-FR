...
---
# <a name="calling-a-stored-procedure-with-a-command"></a><span data-ttu-id="53bbc-102">Appel d’une procédure stockée à l’aide d’une commande</span><span class="sxs-lookup"><span data-stu-id="53bbc-102">Calling a stored procedure with a command</span></span>


<span data-ttu-id="53bbc-103">**S’applique à**: Access 2013, Office 2013</span><span class="sxs-lookup"><span data-stu-id="53bbc-103">**Applies to**: Access 2013, Office 2013</span></span>

<span data-ttu-id="53bbc-p101">Vous pouvez aussi utiliser une commande lorsque vous appelez une procédure stockée. Le code suivant appelle une procédure stockée dans l'exemple de base de données Northwind, appelée CustOrdersOrders, qui est définie comme suit :</span><span class="sxs-lookup"><span data-stu-id="53bbc-p101">You can also use a command when calling a stored procedure. The following code calls a stored procedure in the Northwind sample database, called CustOrdersOrders, which is defined as follows:</span></span>
