---
title: Maison d'édition
manager: kelbow
ms.date: 12/13/2017
ms.audience: Developer
localization_priority: Normal
ms.assetid: b0f11822-d45e-48e6-bf59-990164e3d6d9
description: Trouvez des procédures, des articles techniques et des références pour le développement de solutions et la personnalisation de Publisher.
ms.openlocfilehash: ac2790d49cb324088f19c916723f151e2f288967
ms.sourcegitcommit: 9d60cd82b5413446e5bc8ace2cd689f683fb41a7
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 06/11/2018
ms.locfileid: "19787942"
---
# <a name="publisher"></a>Maison d'édition

Trouvez des procédures, des articles techniques et des références pour le développement de solutions et la personnalisation de Publisher.

> [!NOTE]
> [!REMARQUE] Vous voulez développer des solutions qui étendent l'expérience Office sur [plusieurs plateformes](https://docs.microsoft.com/en-us/office/dev/add-ins/overview/office-add-in-availability) ? Consultez le nouveau [modèle de compléments Office](https://docs.microsoft.com/en-us/office/dev/add-ins/overview/office-add-ins). 

### <a name="viewed-most"></a>Afficher plus

- [Référence VBA pour Publisher](https://msdn.microsoft.com/en-us/VBA/VBA-Publisher)

- [Publisher 2010](https://msdn.microsoft.com/en-us/library/office/ff604963(v=office.14).aspx)


### <a name="office-add-ins"></a>Compléments Office 

- [Prise en main](https://docs.microsoft.com/en-us/office/dev/add-ins/)

- [Présentation de la plateforme](https://docs.microsoft.com/en-us/office/dev/add-ins/overview/office-add-ins)


### <a name="other-resources"></a>Autres ressources

- [Référence de la bibliothèque d'objets VBA Office](https://msdn.microsoft.com/en-us/VBA/Office-Shared-VBA/articles/office-vba-object-library-reference)

- [Référence de langage VBA pour Office](https://msdn.microsoft.com/en-us/VBA/VBA-Language-Reference) 

- [Ouvrir le Kit de développement logiciel (SDK) XML](https://msdn.microsoft.com/en-us/library/bb448854.aspx)

- [Compléments VSTO pour Office](https://msdn.microsoft.com/en-us/library/jj620922.aspx)